...
// Check if user has a specific permission (via custom permissions if set,
// otherwise falling back to the role's legacy defaults)
export const hasPermission = (user, permission) => {
  ...
  // If this account has its own custom permissions configured, that list is
  // authoritative — don't also OR in the role's generic defaults below, or
  // unchecking a permission in the Staff form would have no effect for any
  // role name that happens to match one of the legacy roles further down.
  if (user.permissions && Array.isArray(user.permissions) && user.permissions.length > 0) {
    return user.permissions.includes(permission)
  }
  
  // No custom permissions set — fall back to role-based permissions
